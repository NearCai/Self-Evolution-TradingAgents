...
def _get_YFin_data_from_cache(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Return cached OHLCV rows for ``symbol`` in the requested inclusive range."""
    data = load_ohlcv(symbol, end_date).copy()
    ...
    if data.empty:
        ...
        raise NoMarketDataError(
            symbol, canonical, f"no cached rows between {start_date} and {end_date}"
        )
